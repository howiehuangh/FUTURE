...
class Stream_Analyzer:
    ...
    def __init__(self,
        ...
        self.color_pattern = color_range
    ...
    def update_features(self, n_bins = 3):
        ...
        self.strongest_amplitude  = np.argmax(self.fft)
    ...
    def frequency_to_color(self,freq_range,amplitude_threshold=50, color_pattern=None):
        if color_pattern == None:
            color_pattern = self.color_pattern

        if self.strongest_amplitude <= amplitude_threshold:
            return None

        color_ratio = (freq_range[1]-freq_range[0])/len(color_pattern)
        color_index = int((self.strongest_frequency - freq_range[0]) / color_ratio)
        
        

        if color_index < 0 or color_index >= len(color_pattern):
            return None
        else:
            return self.color_pattern[color_index].hex_l
    ...
    def get_audio_features(self):
        ...
        return self.strongest_frequency